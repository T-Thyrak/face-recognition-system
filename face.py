...
relative_model_path = "svm_model_hog_64_128_aug_upto_160.pkl"
...
# Define the relative path to your PCA model file
relative_pca_model_path = "pca_model.pkl"

# Construct the absolute path to the model file
absolute_pca_model_path = os.path.join(os.path.dirname(current_script_path), relative_pca_model_path)
# Load the trained SVM model
with open(absolute_pca_model_path, 'rb') as pca_model_file:
    pca = pickle.load(pca_model_file)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button('Show result'):
            ...
            reduced_features = pca.transform([hog_features])

            # Make prediction using the trained SVM model
            prediction = clf.predict(reduced_features)[0]
            probability_estimates = clf.predict_proba(reduced_features)
